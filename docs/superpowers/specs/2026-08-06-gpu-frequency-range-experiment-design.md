...
“性能与温控”不再提供独立的 GPU 上限滑块。SIOP 开关启用时固定将三星 GPU 温控
上限放行到设备最高频点。实验开关默认关闭，范围默认保存为 `80-1000 MHz`；关闭时
实验室双端滑块不可操作，也不持有任何频率投票。
...
SIOP 固定放行到设备最高频点，避免与实验室重复提供第二套 GPU 上限。实验启用后的
实际范围只由独立最低、最高投票负责。
